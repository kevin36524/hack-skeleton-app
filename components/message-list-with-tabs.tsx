...
import { Star, Paperclip, Reply, Forward, AlertCircle, Clock, Inbox as InboxIcon } from 'lucide-react';
import { useRecentMessages } from '@/lib/hooks/use-yahoo-mail';

interface MessageListWithTabsProps {
  mailboxId: string;
  folderId: string;
...
export function MessageListWithTabs({
  mailboxId,
  folderId,
  inboxFolderId,
  onMessageSelected,
  selectedMessageId
}: MessageListWithTabsProps) {
  const [folderMessages, setFolderMessages] = useState<Message[]>([]);
  const [folderConversations, setFolderConversations] = useState<Conversation[]>([]);
  const [folderLoading, setFolderLoading] = useState(true);
  const [folderError, setFolderError] = useState<string | null>(null);

  console.log('[MESSAGE LIST TABS] Props:', { mailboxId, folderId, inboxFolderId });

  // Use the hook for recent messages
  const recentMessagesQuery = useRecentMessages(mailboxId, inboxFolderId);

  console.log('[MESSAGE LIST TABS] Recent messages query status:', {
    isLoading: recentMessagesQuery.isLoading,
    isError: recentMessagesQuery.isError,
    error: recentMessagesQuery.error,
    dataLength: recentMessagesQuery.data?.messages?.length
  });
  ...
  const loadFolderMessages = useCallback(async () => {
    try {
      ...
      setFolderConversations(data.conversations);
      setFolderError(null);
    } catch (err) {
      setFolderError('Failed to load messages');
  ...
  return (
    <Tabs defaultValue="folder" className="w-full h-full flex flex-col">
      ...
      <TabsContent value="folder" className="flex-1 overflow-auto px-4 mt-2">
        <MessageListContent
          ...
          onRetry={loadFolderMessages}
        />
      </TabsContent>

      <TabsContent value="recent" className="flex-1 overflow-auto px-4 mt-2">
        <MessageListContent
          messages={recentMessagesQuery.data?.messages || []}
          conversations={recentMessagesQuery.data?.conversations || []}
          loading={recentMessagesQuery.isLoading}
          error={recentMessagesQuery.error ? 'Failed to load recent messages' : null}
          onMessageSelected={onMessageSelected}
          selectedMessageId={selectedMessageId}
          onRetry={() => recentMessagesQuery.refetch()}
        />
      </TabsContent>
    </Tabs>
  );
